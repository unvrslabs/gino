#!/bin/bash
# Pulsar: prepara la compilazione sui Mac di GitHub.
#
...
REPO="unvrslabs/pulsar"
CONFIG="$HOME/Developer/_segreti/pulsar-configuration.json"
CHIAVE_ASC="$HOME/.config/asc/AuthKey_4GAYKQZSHX.p8"
KID="4GAYKQZSHX"
ISS="6e1d4af9-8b65-4825-80f6-53080107e596"

cd "$HOME/Developer/pulsar-ios"
umask 077
LAVORO=$(mktemp -d)
...
if gh repo view "$REPO" >/dev/null 2>&1; then
  ...
  gh repo fork TelegramMessenger/Telegram-iOS --fork-name pulsar \
    --clone=false --default-branch-only
  sleep 8
fi

echo "2/5  carico le nostre modifiche"
git remote remove pulsar 2>/dev/null || true
git remote add pulsar "https://github.com/$REPO.git"
git push -q pulsar HEAD:refs/heads/pulsar --force
gh repo edit "$REPO" --default-branch pulsar >/dev/null 2>&1 || true
echo "     ramo 'pulsar' caricato"
